...
- STD's `Auto-Translate to Chinese` switch is page-scoped and may default to on for each new episode. Verify it is off on that episode detail page before clicking Spotify's `Transcript` tab; turning it off after the tab opens does not undo a translation already sent to the background queue.
...
- Every English evidence anchor must preserve a transcript-verifiable source sentence and be followed immediately by its matching Chinese translation on a separate italicized line, with no prefix label such as `中文翻译：` or `中文解释：`. Do not duplicate Chinese-source anchors as translations. Treat this as a hard preview and delivery gate, enforced in both Markdown review and final DOCX audit.
- Evidence density is a hard gate: keep at most 8 evidence anchors per episode. Prefer a small set covering a concrete mechanism, number/example, constraint, decision, and strategic implication; never retain a long timestamp dump merely because synthesis produced it. Remove paired translation lines together with any discarded anchor.
...
- PDF rendering is a delivery gate: use the accepted Microsoft Word native export path for final PDFs. The ReportLab fallback is preview-only and must not be delivered because it can change CJK rendering, emphasis, heading flow, and pagination. Check screenshots of the title/episode evidence/Part 3/Part 4/Part 5 pages; Markdown structural checks alone are insufficient.
- Markdown emphasis delimiters are not deliverable text. Before final export, verify that `_..._`, `*...*`, and similar markers are rendered as formatting rather than visible characters; every translation must appear as real italics in the PDF.
...
- Transcript language coverage must never infer a match when both the report source and Chinese transcript contain Spotify episode IDs that differ. Exact IDs are required; an ID conflict is a missing Chinese transcript even when titles are substrings or highly similar. Only use title/show fallback when one side has no episode ID, and verify ambiguous matches manually.
...
- After cleanup, run `scripts/audit_transcript_archive_integrity.py <evidence-pack> --require-clean`. Do not mark seen unless every episode has exactly one original transcript and one complete Chinese transcript, with zero missing, duplicate, or incomplete IDs.
